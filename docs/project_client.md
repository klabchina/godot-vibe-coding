...
| 类型 | HP | 速度 | 伤害 | 特性 |
|------|-----|------|------|------|
| **Slime** | 20 | 60 | 5 | 近战：进入半径40px范围后压缩弹跳攻击，动画50%处判定伤害，攻击中禁止移动 |
| **Skeleton** | 40 | 90 | 8 | 远程：侧向游荡2秒→瞄准0.4秒→发射单支常速箭(280px/s) |
| **Orc** | 80 | 50→250 | 15 | 近战+冲锋：每2-7秒随机间隔加速冲刺(5x速度，0.5-1.4秒)，冲刺结束后衔接正面矩形横扫攻击，攻击中禁止移动 |
| **Elite** | 150 | 70 | 12 | 远程：侧向游荡2.5秒→瞄准0.6秒→扇形发射2-4支加速箭 |
| **Boss** | 500 | 40 | 25 | 三阶段：追击→召唤小怪→狂暴加速；近战攻击：进入正面矩形范围后拍击，动画50%处判定伤害 |

### 怪物近战攻击系统

详见 [2026-04-18-melee-attack-design.md](../superpowers/plans/2026-04-18-melee-attack-design.md)

设计要点：
- **攻击范围**：支持圆形（Slime）和正向矩形（Orc/Boss）两种范围定义
- **伤害时机**：精确在攻击动画时长的50%处触发（不是开始、不是结束）
- **移动锁定**：攻击动画播放期间怪物速度清零，不能移动
- **一次性判定**：`DamageApplied` 标志确保每次攻击只造成一次伤害
