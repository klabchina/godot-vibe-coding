...
**项目现状**: Phase 1-6 全部完成，共 64 个 C# 文件 + 4 个场景文件。单机完整游戏体验和网络多人框架均已实现。

| Phase | 状态 | 文件数 |
|-------|------|--------|
| Phase 1: 基础架构层 | ✅ 完成 | ~12 |
| Phase 2: 核心战斗循环 | ✅ 完成 | ~14 |
| Phase 3: 经验升级系统 | ✅ 完成 | ~4 |
| Phase 4: 高级战斗机制 | ✅ 完成 | ~8 |
| Phase 5: UI 与场景流转 | ✅ 完成 | ~9 |
| Phase 6: 网络多人对战 | ✅ 完成 | 7 |
...
- [x] 项目可编译通过（`dotnet build`）
- [x] 所有 Data 类的公式方法可通过手动测试验证（如 `MonsterData.GetHp(MonsterType.Slime, 3)` 返回 24）
- [x] ECS World 可创建 Entity、添加/获取 Component、注册/执行 System
...
- [x] 玩家可 WASD 移动，弓箭自动射击最近怪物
- [x] 怪物从四边刷新，追踪玩家，被箭射死后消失
- [x] 波次自动推进（Wave 1 → Wave 7），每波间歇 5 秒
- [x] 玩家被怪物接触会扣血，HP 降至 0 游戏结束
- [x] BattleHud 显示波次和 HP
...
- [x] 怪物死亡掉落绿色经验球，玩家靠近自动吸附拾取
- [x] 经验达到阈值弹出 3 选 1 升级面板
- [x] 选择多重射击后可见箭矢数增加
- [x] 选择射速/伤害/穿透后效果生效
- [x] 升级面板 5 秒倒计时 + 超时自动选择
- [x] 单局最高 Lv8，满级后不再弹出面板
...
- [x] 所有 14 种升级效果均可正常生效
- [x] Boss 三阶段切换正常（追击→召唤→狂暴）
- [x] Skeleton 闪避、Orc 冲锋行为正常
- [x] 道具掉落并拾取生效
- [x] Buff 图标显示、计时正确、新覆盖旧
- [x] 完整单人 8 波战斗可通关
...
- [x] 可从 MainMenu 开始，经完整 8 波战斗，到 Result 结算，再返回 MainMenu
- [x] 结算界面正确显示波次、击杀、伤害、评分
- [x] 战斗 HUD 显示 BuffBar、UpgradeBar
- [x] 伤害飘字效果正常
